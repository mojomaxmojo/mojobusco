 ...
 *
 * PERFORMANCE:
 * - React.memo: Parent-Re-Renders (Zap-Polling auf Detailseiten, Social-Count-
 *   Updates, Filter-State) führen NICHT mehr zum Neuparsen des Markdowns.
 *   react-markdown v10 parst bei jedem Render neu – wer nicht re-rendert, parst nicht.
 * - components-Objekt via useMemo: nur 2 Identitäten (mit/ohne hideImageLinks)
 *   statt einem neuen Objekt bei jedem Render.
 * - Content-Vorverarbeitung (hideImageLinks-Regex-Kette) via useMemo: läuft nur
 *   bei echtem Content-/Flag-Wechsel.
 * Props-Stabilität: Alle Aufrufer übergeben event aus dem TanStack-Query-Cache
 * (stabile Referenzen) und className als Literal → memo greift zuverlässig.
 */
export const NoteContent = memo(function NoteContent({
  ...
  // Content aufbereiten (nur neu bei echtem Content-/Flag-Wechsel)
  const content = useMemo(() => {
    const raw = event?.content ?? '';

    if (!hideImageLinks) return raw;

    // Remove all image/video URLs from content
    let cleaned = raw.replace(/https?:\/\/[^\s]+\.(jpg|jpeg|png|gif|webp|mp4|webm|mov|avi|mkv)/gi, '');
    // Remove blossom, mojobus and other media hosting URLs
    cleaned = cleaned.replace(/https?:\/\/[^\s]*(blossom|nostr\.build|imgur\.com|mojobus\.co|cdn\.blossom)[^\s]*/gi, '');
    ...
    return cleaned;
  }, [event?.content, hideImageLinks]);

  // Renderer-Map: Identity wechselt nur beim Wechsel von hideImageLinks
