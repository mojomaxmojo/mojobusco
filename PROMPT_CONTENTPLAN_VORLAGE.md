...
> Muster von `CONTENTPLAN_FIGUEIRA_BUDENS.md` / `CONTENTPLAN_MANTA_ROTA.md`
> (30 oder 15 Artikel, 8 bzw. 4 Wochen, Top-SEO-Briefs) — **inkl. fertigem
> JSON-Block**, der direkt als `public/data/contentplans/<id>.json` speicherbar ist.
  ...
  "articles": [
    {
      ...
      "star": <true bei Top-Brief-Artikeln, sonst false/weglassen>,
...
1. Block oben kopieren, AUSFÜLLEN-Block anpassen (Ort, Zeitraum, Fokus, UMFANG).
2. In eine neue AI-Session einfügen — die AI recherchiert (FAKTEN + SERP-Check)
   und liefert Markdown-Plan + JSON-Block.
3. JSON-Block als `public/data/contentplans/<id>.json` speichern + Index-Eintrag
   (oder dem Shakespeare-Assistenten zeigen — der baut es ein).
...
5. GSC-Loop: nach Woche 3/6 echte Queries aus dem Ranking-Block holen →
   Roadmap-Tail dynamisch ersetzen. Refresh-Stempel im Folgejahr nutzen →
   Saison-Artikel aktualisieren statt neu schreiben.

**UMFANG 15 (Minimal-Variante, geringer Aufwand):**
6 Top-Briefs (Pillars/Listicle) + 6 Guides (M) + 3 Erlebnisse (K) +
1 GSC-Dynamik-Slot, 4 Wochen, ~8–10 Places, 2–3 Trips. Alle Regeln
identisch — nur kleiner. Wählt diese Variante, wenn der Aufwand pro Ort
maximal gering sein soll (z. B. kurze Zwischenstopps).
...
FOKUS: Wandern, Strände, Überwintern · UMFANG: 30 · ANZAHL ARTIKEL: 30
```
