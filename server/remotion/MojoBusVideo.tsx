    ...
    <AbsoluteFill style={{ background: '#000' }}>
      ...
      <ColorGradeWrapper grade={grade}>
        ...
        {slideDefs.map((def, i) => {
          ...
          const normalPunch = !isRoute && fx.zoomPunchScale > 0 && cutFx[i] !== 'whip' && i > 0;
          const heroWordForSlide = heroWordWindows.find(w => w.slideIndex === i);
          const heroWordOffset = heroWordForSlide ? heroWordForSlide.startFrame - absoluteStart : undefined;
          ...
          // ⬇️ Hero-Word-Punch (inner, am Wort-Offset mitten im Slide)
          if (heroWordOffset !== undefined) {
            slideContent = (
              <ZoomPunchWrapper punchScale={0.08} punchDelay={heroWordOffset}>
                {slideContent}
              </ZoomPunchWrapper>
            );
          }
          // ⬇️ Normaler Cut-Punch (außen, am Cut = frame 0)
          if (normalPunch) {
            slideContent = (
